...
################################################################################
# Get and format truthtable from a file -- USED FOR EDGE DETECTION IN JETCAS'19 PAPER
def get_tt_from_file(fname):
    tt_list = []
    out_list = []
    with open(fname, 'r') as f:
        lines = f.readlines()
    for l in lines:
        input_str = l.strip().split(',')[0]
        output_str = int(l.strip().split(',')[1])
    
        nbits = (len(input_str)-2)/4

        A = input_str[2:10]
        B = input_str[10:18]
        neg_A = input_str[18:26]
        neg_B = input_str[26:34]

        weaved_str = '01'
        for i in range(nbits):
            weaved_str += A[i]+neg_A[i]+B[i]+neg_B[i]
    
        print input_str, weaved_str
        tt_list.append(weaved_str)
...
# get truthtable for distribution based edge detection
# the variables are still called 'adder' since it's easier to handle them within the objective function as global variables
# name stays the same, content changes depending on the function to fetch the truthtable
adder_truthtable, adder_msb = get_tt_from_file('new_tt_fgt3_16.txt') 
